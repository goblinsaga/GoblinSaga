...
import RewardsContract2ABI from '../../contracts/RewardsContract2ABI.json';
...
const contractAddress = '0xB787f894a5a6475Bc1964351650B8746CB9368bB';

const ClaimRewardsButtonTopTwo = () => {
  ...
  const connectWallet = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        const contract = new ethers.Contract(contractAddress, RewardsContract2ABI, signer);
  ...
  return (
    <div>
      {!account ? (
        <button style={{ width: "150px" }} className="metaportal_fn_buttonLW " onClick={connectWallet} disabled={connecting} >
          {connecting ? 'Connecting...' : 'Connect'}
        </button>
      ) : (
        <button style={{ width: "150px" }} className="metaportal_fn_buttonLW" onClick={handleClaimRewards} disabled={claiming}>
...
export default ClaimRewardsButtonTopTwo;